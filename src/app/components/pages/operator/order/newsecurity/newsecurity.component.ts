...
@Component({
    selector: 'page-newsecurity-order',
    templateUrl: './newsecurity.component.html',
    styleUrls: ['./newsecurity.component.scss']
})
export class OperatorNewSecurityOrderComponent implements OnInit {
    ...
    change() {
        ...
        this.api.getFISSecurityDetailByISIN(isin).subscribe(res => {
            if (res.success == false) {
                ...
                console.log('success');
                this.form.controls['security_description'].setValue(res.name);
                this.form.controls['operator_price'].setValue(res.coupon_rate);

                if (res.currency) {
                    let selected_currency = this.currencies.filter(currency => currency.code == res.currency);
                    if (selected_currency.length > 0) {
                        this.form.controls['txn_currency'].setValue(selected_currency[0]['id']);
                    } else {
                        this.form.controls['txn_currency'].setValue('');
                    }
                }
            }
        })
